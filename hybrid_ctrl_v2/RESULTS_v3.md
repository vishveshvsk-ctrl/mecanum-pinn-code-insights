...
**Headline: PID-CT wins in all three.** ASMC and PID-FB are tied except under noisy-tuned
oracle feedback; noisy tuning does not transfer to ESKF feedback, so the CLEAN-tuned
configs are the ones to deploy.
...
- **`:test_v3` generalisation** — the tier exists but no v3 config has been evaluated on it.
  All numbers above are on the training tier `train14_v3`.
