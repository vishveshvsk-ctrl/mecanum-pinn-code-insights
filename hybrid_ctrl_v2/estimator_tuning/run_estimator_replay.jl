#!/usr/bin/env julia
# =============================================================================
# hybrid_ctrl_v2/estimator_tuning/run_estimator_replay.jl — CLI entry point
# =============================================================================
# Tunes ESKFEstimatorV2's 10-dim param space (P0_vel/yaw/heading/bias_acc/
# bias_gyro/slip/pos, pose_Qn_heading/pos, slip_R_inflate) by REPLAYING the
# candidate filter over pre-simulated Arrow trajectories already in
# data/Simulation_Data_MecanumSlipSpin_LugreAdamov/ (no plant/controller ODE
# solve per candidate -- per-user direction). Reuses
# hybrid_ctrl_v2/controller_tuning/optimizer_stage.jl's generic
# `optimize_staged` (dxNES global search, then BOBYQA local refine) unmodified.
#
# Since replay evaluations are cheap (no ODE solve), there is no
# screen/full-tier split here -- both optimizer phases score against the
# full 11-trajectory replay_trajset().
#
...
function parse_args(argv)
    a = Dict{String,Any}("seed" => 1, "p1-cap" => 250, "p2-cap" => 60,
                         "out" => "hybrid_ctrl_v2/runs_estimator_v2_replay", "smoke" => false,
    ...
    while i <= length(argv)
        ...
        else; error("run_estimator_replay.jl: unknown arg $arg"); end
    end
    return a
end
...
function run_smoke(space, trajs)
    ...
    obj = make_replay_objective_v2(space, trajs; seed=1)
    ...
    r.n_fail == 0 || error("run_estimator_replay.jl --smoke: $(r.n_fail)/$(length(trajs)) trajectories failed")
    println("=== SMOKE OK ===")
end
...
function main()
    ...
    lo, hi = space.flat_lower, space.flat_upper
    obj = make_replay_objective_v2(space, trajs; seed=a["seed"])

    outdir = joinpath(a["out"], "seed$(a["seed"])")
    mkpath(outdir)

    println("\n===== ESKFEstimatorV2 replay tuning / seed $(a["seed"]) — $(length(trajs)) trajectories =====")
    ...
    println("[eskf_v2 replay seed$(a["seed"])] best score=$(round(best_score,digits=4)) " *
