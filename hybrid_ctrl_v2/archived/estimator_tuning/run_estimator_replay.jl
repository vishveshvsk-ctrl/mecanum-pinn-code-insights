...
function run_smoke(space, trajs)
    println("=== SMOKE: default-gains replay eval on $(length(trajs)) segments ===")
    ...
    r.n_fail == 0 || error("run_estimator_replay.jl --smoke: $(r.n_fail)/$(length(trajs)) segments failed")
    println("=== SMOKE OK ===")
end
...
function main()
    ...
    println("\n===== ESKFEstimatorV2 replay tuning / seed $(a["seed"]) — $(length(trajs)) segments (realistic noise) =====")
