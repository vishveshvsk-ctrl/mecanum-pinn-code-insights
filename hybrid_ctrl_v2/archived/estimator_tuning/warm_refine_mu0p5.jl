#!/usr/bin/env julia
# =============================================================================
# hybrid_ctrl_v2/estimator_tuning/warm_refine_mu0p5.jl — warm-started BOBYQA
# continuation for the mu=0.5 train12 realistic-noise run.
# =============================================================================
# Same contract as warm_refine.jl, but uses mu0p5_train12_replay_trajset().
#
#   --seeds       comma-separated seed list (default 1,2,3)
#   --cap         new BOBYQA eval cap (default 150)
#   --in          input root (default hybrid_ctrl_v2/runs_estimator_v2_mu0p5_train12)
...
function parse_args(argv)
    a = Dict{String,Any}("seed" => 1, "cap" => 150,
                         "in" => "hybrid_ctrl_v2/runs_estimator_v2_mu0p5_train12",
                         "out" => "hybrid_ctrl_v2/runs_estimator_v2_mu0p5_train12",
    ...
    while i <= length(argv)
        ...
        else; error("warm_refine_mu0p5.jl: unknown arg $arg"); end
    end
    return a
end
...
function main()
    ...
    println("\n===== warm-refine mu0p5 train12 seed $(a["seed"]) — prev best_score=$(round(prev_score,digits=4)), " *
            "cap=$(a["cap"]) =====")
    ...
    println("[warm mu0p5 train12 seed$(a["seed"])] prev=$(round(prev_score,digits=4)) -> " *
